...
-- | the application monad, `BuildX.AppM`.
-- |
...
module BuildX.Capability.LogMessages where

import Prelude

import BuildX.Capability.Now (class Now)
import BuildX.Data.Log (Log, LogReason(..), mkLog)
...
-- | `Log` type from `BuildX.Data.Log`. This type can only be constructed using helper functions
