...
-- | monad, `BuildX.AppM`.
module BuildX.Api.Utils where

import Prelude

import Affjax (request)
import BuildX.Api.Request (BaseURL, RequestOptions, Token, defaultRequest, readToken, writeToken)
import BuildX.Capability.LogMessages (class LogMessages, logError)
import BuildX.Capability.Now (class Now)
import BuildX.Data.Profile (Profile)
import BuildX.Data.Username (Username)
import BuildX.Env (UserEnv)
...
-- | `baseUrl` field from the app environment. See `BuildX.AppM` for examples of this in action.
mkRequest
  :: forall m r
   . MonadAff m
...
-- | the asynchronous request. See `BuildX.AppM` for examples of this in action.
mkAuthRequest
  :: forall m r
   . MonadAff m
  => MonadAsk { baseUrl :: BaseURL | r } m
...
-- | dealing with malformed responses. See `BuildX.AppM` for examples of this in practice.
