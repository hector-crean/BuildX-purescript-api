...
-- | functions that our production monad will leverage in `BuildX.AppM` to implement our various
-- | app capabilities.
module BuildX.Api.Request
...
import BuildX.Api.Endpoint (Endpoint(..), endpointCodec)
import BuildX.Data.Email (Email)
import BuildX.Data.Email as Email
import BuildX.Data.Profile (Profile)
import BuildX.Data.Profile as Profile
import BuildX.Data.Username (Username)
import BuildX.Data.Username as Username
...
-- | BuildX uses a REST API and secures certain endpoints using a JSON Web Token (JWT). That means
...
-- |
-- | Check out the `BuildX.Capability.Resource.*` modules for examples of the high-level functions
...
-- | request functions defined below and in the `BuildX.Api.Utils` modules.
...
decodeAuthProfile { user } = do
  ...
  where
  tokenCodec =
    CA.prismaticCodec "Token" (Just <<< Token) (\(Token t) -> t) CA.string

-- | The following functions deal with writing, reading, and deleting tokens in local storage at a
-- | particular key. They'll be used as part of our production monad, `BuildX.AppM`.
